...
def rebuild() -> None:
    ...
    aligned = _import_or_path("build_player_streaks_and_aligned", "build_player_streaks_and_aligned.py", required=True)
    log.info("→ build_player_streaks_and_aligned.main(rebuild=True)")
    aligned.main(rebuild=True, restrict_age_25_29=False)

    # 3) Player z (optional)
    z_player = _import_or_path("build_player_five_year_aligned_z", "build_player_five_year_aligned_z.py", required=False)
    if z_player:
        log.info("→ build_player_five_year_aligned_z.main(upsert)")
        z_player.main(mode="upsert")

    # 4) Cohort z (required)
    z_cohort = _import_or_path("build_player_five_year_aligned_z_cohort", "build_player_five_year_aligned_z_cohort.py", required=True)
